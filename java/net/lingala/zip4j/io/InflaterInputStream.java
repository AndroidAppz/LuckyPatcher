...
public class InflaterInputStream extends PartInputStream {
    private byte[] buff = new byte[LZMA2Options.DICT_SIZE_MIN];
    ...
    private void finishInflating() throws IOException {
        do {
        } while (super.read(new byte[Util.BLOCK_HEADER_SIZE_MAX], 0, Util.BLOCK_HEADER_SIZE_MAX) != -1);
    ...
    public long skip(long n) throws IOException {
        ...
        int total = 0;
        byte[] b = new byte[ResConst.RES_TABLE_PACKAGE_TYPE];
        while (total < max) {
            int len = max - total;
            if (len > b.length) {
                len = b.length;
